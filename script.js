...
// Helper function to get a random element from the words array
function randomWords(numberOfWords = 5) {
  let randomWords = [];
  for (let i = 0; i < numberOfWords; i++) {
    randomWords.push(wordList[Math.floor(Math.random() * wordList.length)]);
  }
  return randomWords.join(' '); // Join words into a sentence.
}
...
const copyToClipboard = () => {
  ...
  navigator.clipboard
    ...
    .then(() => {
      // Optional: Add some user feedback here, like changing the button text to "Copied!" temporarily
